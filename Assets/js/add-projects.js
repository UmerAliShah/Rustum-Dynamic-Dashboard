document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function uploadImage(file) {
    try {
      ...
      const {
        data: { publicUrl },
      } = window.supabase.storage.from("project-images").getPublicUrl(fileName);
  ...
  window.deleteProject = async function (id) {
    ...
    try {
      ...
      const deletePromises = images.map((img) => {
        const fileName = img.image_url.split("/project-images/")[1];
      ...
      // 5. Delete project
      const { error: projectDeleteError } = await window.supabase
        .from("projects")
        .delete()
        .eq("id", id);
    ...
    try {
      ...
      renderProjects(
        ...
        document.getElementById(
          type === "completed" ? "completedList" : "currentList"
        ),
        type
      );
    } catch (err) {
