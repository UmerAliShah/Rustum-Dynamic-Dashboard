document.addEventListener("DOMContentLoaded", async function() {
    ...
    async function uploadImages(files, type) {
        ...
        for (const file of files) {
            try {
                const fileName = `${Date.now()}_${type}_${file.name.replace(/\s+/g, '_')}`;
    ...
    function renderList(entries, containerId, type) {
        ...
        container.innerHTML = entries.map(entry => `
            <div class="entry-card">
                ...
                <div class="mt-2">
                    <button class="btn btn-warning btn-sm" onclick="editEntry('${type}', ${entry.id})">Edit</button>
                    <button class="btn btn-danger btn-sm" onclick="deleteEntry('${type}', ${entry.id})">Delete</button>
                </div>
            </div>
        `).join('');
    }
    ...
    window.deleteEntry = async function(type, id) {
        ...
        try {
            ...
            // Delete database entry (will cascade delete images)
    ...
    // Event Listeners
    machineryForm.addEventListener('submit', e => handleFormSubmit(e, 'machinery'));
